...
def cal_anomaly_map(ft_list, fs_list, out_size=224, amap_mode='mul', enhance=True, alpha=1):
    if amap_mode == 'mul':
...
def evaluation_multi_proj(encoder,bn, decoder, dataloader,device):
    ...
    with torch.no_grad():
        for (img, gt, label, _, _) in dataloader:
            img = img.to(device)
            inputs = encoder(img)
            outputs = decoder(bn(inputs),flag=1)
            anomaly_map, _, anomaly_map_enhance = cal_anomaly_map(inputs, outputs, img.shape[-1], amap_mode='a')
